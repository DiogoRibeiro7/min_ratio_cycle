...
class BenchmarkRunner:
    ...
    def scaling_benchmark(self, max_vertices: int = 100, step: int = 10):
        ...
        for n in sizes:
            ...
            for graph_type, generator in [
                ...
                (
                    "complete",
                    lambda: GraphGenerator.complete_graph(min(n, 15)),
                ),  # Cap complete graphs
            ]:
                ...
                try:
                    ...
                    if result.success:
                        print(
                            f"    {graph_type}: {result.solve_time:.4f}s, "
    ...
    def mode_comparison_benchmark(self):
        ...
        for n in sizes:
            ...
            for u, v, c, t in edges:
                solver_numeric.add_edge(
                    u, v, float(c) + 0.1, float(t)
                )  # Add small float
            ...
            if result_exact.success and result_numeric.success:
                ...
                print(
                    f"    Exact:   {result_exact.solve_time:.4f}s, ratio={result_exact.ratio:.6f}"
                )
                print(
                    f"    Numeric: {result_numeric.solve_time:.4f}s, ratio={result_numeric.ratio:.6f}"
                )
    ...
    def generate_report(self, save_plots: bool = True):
        ...
        print(
            f"Success rate: {success_rate:.1f}% ({len(successful)}/{len(self.results)})"
        )
        ...
        if exact_results and numeric_results:
            ...
            print(
                f"  Numeric mode: {len(numeric_results)} runs, mean time: {np.mean(numeric_times):.4f}s"
            )
